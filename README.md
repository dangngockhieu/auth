...
- `format_response` format_response trả về
- `exception` gom custom exception và global handler để API trả lỗi nhất quán.

Áp dụng SOLID ngắn gọn:

- `S`: mỗi lớp một trách nhiệm rõ ràng, ví dụ `JwtTokenProvider` chỉ xử lý JWT
- `O`: thêm role, thêm auth provider hoặc thay đổi response mapper không phải sửa controller.
- `L`: interface `AuthService` có thể thay implementation mà không đổi nơi sử dụng.
- `I`: không ép controller/repository phụ thuộc logic dư thừa.
- `D`: service phụ thuộc abstraction (`AuthService`) và bean injected qua constructor.
...
- `account`: dùng access token đã được filter xác thực để lấy profile hiện tại.
...
Description: Cấp lại Access Token mới khi token cũ hết hạn. Trình duyệt sẽ tự động đính kèm Refresh Token thông qua Cookie.

Headers:

- Content-Type: application/json
- Cookie: refreshToken=<jwt-refresh-token> (Trình duyệt tự quản lý)
...
curl --request POST 'http://localhost:8080/api/auth/refresh' \
  --header 'Cookie: refreshToken=<jwt-refresh-token>'
```
...
```bash
curl --request GET 'http://localhost:8080/api/auth/account' \
  --header 'Authorization: Bearer <jwt-access-token>'
```
...
- `scripts/curl-auth-smoke.sh`: chạy full flow `register -> login -> account -> refresh -> logout -> verify refresh revoked`
...
```bash
TEST_NAME='Dang Ngoc Khieu'
TEST_EMAIL='khieu.test@example.com'
